...
const home = {
  template: `
    <div class="vh-100 d-flex flex-column align-items-center justify-content-center">
      <h1>Examen UF4</h1>
      <del>Dios es real, a no ser que lo declares como integer.</del>
    </div>
...
class Tickets {
  ...
  static async getById(id) {
    const { data: tickets, error } = await supabase.from("tickets").select("*").eq("id", id).single();
    if (error) {
      throw new Error(error.message);
    }
  ...
  static async create(ticketsData) {
    const { error } = await supabase.from("tickets").insert(ticketsData).select();
    if (error) {
      throw new Error(error.message);
    }
    return true;
...
const registro = {
  ...
  <form action="" class="form p-4 border shadow bordered mt-5 mx-auto" style="width: 400px;">
    ...
    <label for="pass" class="mt-2 form-label">Contraseña: </label>
    <input type="text" class="form-control" id="contrasenya">
